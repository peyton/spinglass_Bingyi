...
void make_measure_m(IsingData *ising, MeasureM *measure_m) {
	...
	measure_m->m = measure_m->m + abs((double) mtemp) / ((double) L*L*1);
	measure_m->m2 =measure_m->m2 + pow(((double) mtemp) / ((double) L*L*1),2 );
	measure_m->m4 =measure_m->m4 + pow(((double) mtemp) / ((double) L*L*1),4 );
}
...
/*							+ising->z_interaction[i][j][left]*ising->lattice_site[i][j][left]
	...
	measure_E->E+= tempE / ((double) L*L*1);
	measure_E->E2+= pow(tempE / ((double) (L*L*1)),2) ;
}
